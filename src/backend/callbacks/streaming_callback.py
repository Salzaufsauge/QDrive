...
class StreamingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            log("INFO", f"Evaluation over {self.n_eval_episodes} episodes: {mean_reward:.2f} +/- {std_reward:.2f}")
        ...
        for env_idx, info in enumerate(infos):
            if "episode" in info:
                ...
                log("INFO", f"Episode finished after {episode['timesteps']} timesteps with reward {episode['reward']}.")
